"""
AI Service for generating data dictionary descriptions using Groq
...
class AIService:
    """Service for AI-powered data dictionary generation using Groq"""
...
Provide a clear, helpful response based on the database schema above."""
    ...
    def _build_schema_summary(self, dictionary_context: Dict[str, Any]) -> str:
        ...
        db_name = dictionary_context.get('database_name', 'unknown')
        db_type = dictionary_context.get('database_type', 'unknown')
        summary_parts.append(f"Database: {db_name} ({db_type})")
            ...
            for table_name, table_info in list(tables.items())[:10]:
                if isinstance(table_info, dict):
                    ...
                    if isinstance(columns, list):
                        column_names = [col.get('name', '') if isinstance(col, dict) else str(col) for col in columns[:5]]
                    elif isinstance(columns, dict):
                        column_names = list(columns.keys())[:5]
                    else:
                        column_names = []
        ...
        total_tables = dictionary_context.get('total_tables', 0)
        total_columns = dictionary_context.get('total_columns', 0)
